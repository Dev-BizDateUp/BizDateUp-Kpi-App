    ...
    <>
...

      {
        leadershipboardbadges.length === 0 ? <>
          <h1 className="text-center text-red-500 text-4xl font-bold mt-5">No Badges Yet </h1>
        </> : <>
          <table className="w-full mt-5">
            ...
            <tbody>
              {data.length > 0 ? (
                data.map((item, index) => (
                  <tr key={item.id || index} className={`text-center border-b ${index === 0 || index === 1 || index === 2 ? "bg-[#008000] text-white" : "" }`}>
